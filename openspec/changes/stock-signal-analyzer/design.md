...
1. 顶部一句话结论 + 操作建议
2. 11 个信号各自的红黄绿灯 + 确定度数字 + 进度条 + 文字解释
3. 底部一段综述，把逻辑串起来

## Goals / Non-Goals

### Goals
- 11 个信号各自有独立的确定度计算公式，物理可解释
...
| S9 | 回踩不破 | 触碰支撑 + 收盘守住 + 下影不破 + 支撑质量 | 假破位收回后的二次确认 |
| S10 | MACD金叉 | DIF-DEA 差值归一化 | 金叉后叉开越多越确定 |
| S11 | 低点抬升 | `clamp(抬升幅度/ATR, 0, 1)` | 抬升 1 个 ATR = 满分 |
